  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-4 shadow-sm transition-colors duration-200">
      ...
      <div className="flex flex-col gap-1 mb-3">
        <label className="text-[10px] font-body font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wide">{t('tableFilters.search')}</label>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-3">
        <Select label={t('tableFilters.area')}    allLabel={t('tableFilters.all')} value={filters.programa    || ''} onChange={handlePrograma}    options={programas} />
        <Select label={t('tableFilters.subarea')} allLabel={t('tableFilters.all')} value={filters.subPrograma || ''} onChange={handleSubPrograma} options={subProgramas} />
        <Select label={t('tableFilters.project')} allLabel={t('tableFilters.all')} value={filters.proyecto    || ''} onChange={handleProyecto}    options={proyectos} />
        <Select label={t('tableFilters.task')}    allLabel={t('tableFilters.all')} value={filters.actividad   || ''} onChange={handleActividad}   options={actividades} />
      </div>

      {/* Otros filtros */}
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 items-end">
        <Select label={t('tableFilters.work')}    allLabel={t('tableFilters.all')} value={filters.obra   || ''} onChange={v => onFilterChange('obra',   v)} options={obras} />
        <Select label={t('tableFilters.funding')} allLabel={t('tableFilters.all')} value={filters.fuente  || ''} onChange={v => onFilterChange('fuente', v)} options={fuentes} />
        <Select label={t('tableFilters.type')}    allLabel={t('tableFilters.all')} value={filters.tipo    || ''} onChange={v => onFilterChange('tipo',   v)} options={tipos} />
        ...
        {hasFilters && (
          ...
          >
            <X size={12} /> {t('tableFilters.clear')}
          </button>
        )}
      </div>
    </div>
